...
final class SQLStorageTests: XCTestCase {
    ...
    func testStorageListObjectsPagination() async throws {
        try await withSQLStorage { storage in
            ...
            _ = try await storage.putObject(bucket: "list-bucket", key: "a/1.txt", data: [data].async, size: 1, metadata: nil)
            _ = try await storage.putObject(bucket: "list-bucket", key: "a/2.txt", data: [data].async, size: 1, metadata: nil)
            _ = try await storage.putObject(bucket: "list-bucket", key: "b/1.txt", data: [data].async, size: 1, metadata: nil)
            _ = try await storage.putObject(bucket: "list-bucket", key: "c.txt", data: [data].async, size: 1, metadata: nil)

            // 1. Prefix
            let res1 = try await storage.listObjects(bucket: "list-bucket", prefix: "a/", delimiter: nil, marker: nil, continuationToken: nil, maxKeys: 1000)
            XCTAssertEqual(res1.objects.count, 2)
            XCTAssertEqual(res1.objects[0].key, "a/1.txt")

            // 2. Delimiter
            let res2 = try await storage.listObjects(bucket: "list-bucket", prefix: nil, delimiter: "/", marker: nil, continuationToken: nil, maxKeys: 1000)
    ...
    func testStorageCopyObject() async throws {
        try await withSQLStorage { storage in
            ...
            _ = try await storage.putObject(bucket: "src-bucket", key: "source.txt", data: [data].async, size: Int64(data.readableBytes), metadata: metadata)

            let copyMeta = try await storage.copyObject(fromBucket: "src-bucket", fromKey: "source.txt", toBucket: "dst-bucket", toKey: "copied.txt")
    ...
    func testStorageCreateDeleteBucket() async throws {
        try await withSQLStorage { storage in
            ...
            let result = try await storage.listObjects(bucket: "test-bucket", prefix: nil, delimiter: nil, marker: nil, continuationToken: nil, maxKeys: nil)
    ...
    func testStoragePutGetObject() async throws {
        try await withSQLStorage { storage in
            ...
            _ = try await storage.putObject(bucket: "test-bucket", key: "hello.txt", data: [buffer].async, size: Int64(data.count), metadata: nil)

            let (metadata, body) = try await storage.getObject(bucket: "test-bucket", key: "hello.txt", range: nil)
    ...
    func testStorageMetadata() async throws {
        try await withSQLStorage { storage in
            ...
            _ = try await storage.putObject(bucket: "meta-bucket", key: "obj", data: [data].async, size: Int64(data.readableBytes), metadata: metadata)

            let (readMeta, _) = try await storage.getObject(bucket: "meta-bucket", key: "obj", range: nil)
