...
struct SQLMetadataStore: MetadataStore {
    ...
    static func create(path: String, on eventLoopGroup: EventLoopGroup, threadPool: NIOThreadPool) async throws -> SQLMetadataStore {
    ...
    func getMetadata(bucket: String, key: String) async throws -> ObjectMetadata {
        let query = "SELECT size, last_modified, etag, content_type, custom_metadata FROM objects WHERE bucket = ? AND key = ?;"
        let rows = try await connection.query(query, [SQLiteData.text(bucket), SQLiteData.text(key)])
        ...
        let customMetadata = (try? JSONDecoder().decode([String: String].self, from: Data(customMetadataJSON.utf8))) ?? [:]
    ...
    func listObjects(
        ...
        var query = "SELECT key, size, last_modified, etag, content_type, custom_metadata FROM objects WHERE bucket = ?"
        ...
        for row in rows {
            ...
            let customMetadata = (try? JSONDecoder().decode([String: String].self, from: Data(customMetadataJSON.utf8))) ?? [:]
             ...
             allObjects.append(ObjectMetadata(
                key: key,
                size: Int64(size),
                lastModified: lastModified,
                eTag: eTag,
