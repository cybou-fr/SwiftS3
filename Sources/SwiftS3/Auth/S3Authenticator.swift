...
struct S3Authenticator<Context: RequestContext>: RouterMiddleware {
    ...
    func verifySignature(request: Request, authHeader: String) async throws -> Bool {
        ...
        guard let credentialRange = credentialPart.range(of: "Credential="),
              ...
              let signatureRange = signaturePart.range(of: "Signature=") else {
            return false
        }
        ...
        // query must be sorted by key
        let query = request.uri.queryParameters.map { "\($0.key)=\($0.value)" }.sorted().joined(separator: "&")
        ...
        let payloadHash = request.headers[HTTPField.Name("x-amz-content-sha256")!] ?? "UNSIGNED-PAYLOAD"
        ...
        let canonicalRequestHash = try SHA256.hash(data: Data(canonicalRequest.utf8)).map { String(format: "%02x", $0) }.joined()
        ...
        let calculatedSignature = calculatedSignatureData.map { String(format: "%02x", $0) }.joined()
